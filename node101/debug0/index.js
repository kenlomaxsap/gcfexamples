...
findJson = (json) => {
  if (json && json.body) // when called via "functions call", the json.data is found in body..
    json = json.body
  if (json && json.query) // when called via express json is in query
    json = json.query  
  if (json && json.data) // When called via events (see later), json is in json.data..
    json = json.data;
